...
班表 section 的主官僅限調度主任／調度副主任；領班與實習領班依原始 section 留在各區。原表前段未逐人重複標示的監控列沿用原始表頭區塊，不根據某日班碼判斷。重新產生顯示排序不修改原始 sourceOrder、sourceRow 或日夜來源。

`schedule-display.mjs` 集中管理純標題別名與跳轉大分類。`getScheduleAreaDisplayLabel(section, sourceGroup)` 依原始來源組別顯示：大小夜班 F1→F、I2→I、ZH2→ZH、M2→M、W3→W1，W2 保持 W2；日班及來源不明時不套用這些數字別名，日班 I1／I2／I3、W1／W2／W3 保持原名。呼叫端傳入共用來源資料的 group，不依當日班碼判斷。改名仍以原始區域鍵分組，不用新標題合併或搬移人員。工兵小隊標題完全隱藏車號。J2／晚PT數字的顯示 section 併為府前PT（日班 20 位）；原始晚PT 13 列中有 6 位與府早重複、1 位 B3810 維持既有大小夜班 A1 編制，不跨組搬人。班表、預排與正式班表管理共用上述顯示 helper；派工僅共用導航大分類，不更動派工卡片資料或區域名稱。

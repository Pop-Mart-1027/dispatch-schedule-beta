...
(async () => {
 ...
 for (const width of [320,390,430]) {
  ...
  await loaded;
  assert.equal(await page.getByLabel('員工編號',{exact:true}).inputValue(),'');
  await page.getByPlaceholder('請使用本人員工編號登入').fill('96504');
  await page.getByPlaceholder('首次登入密碼為編號').fill('96504');

...
function FirestoreDispatchView({ employeeId, isDuty }: { employeeId: string; isDuty: boolean }) {
  ...
  return <><div className="dispatch-toolbar"><label>日期<input type="date" value={date} onChange={event => event.target.value && setDate(event.target.value)} /></label><div className="tabs"><button className={shift === 'night' ? 'tab active' : 'tab'} onClick={() => setShift('night')}>夜班</button><button className={shift === 'morning' ? 'tab active' : 'tab'} onClick={() => setShift('morning')}>早班</button></div></div>{error && <div className="result-card result-warning">{error}</div>}{!records.length && !error ? <p className="loading">此日期尚未匯入班表。</p> : !visible.length ? <p className="loading">此日期沒有{shift === 'night' ? '夜班' : '早班'}派工。</p> : <div className="dispatch-grid">{orderedGroups.map(([area, members]) => { const special = area === '單位主官' || area.includes('監控'); return <article className={special ? 'dispatch-card command-card' : 'dispatch-card'} key={area}><header><span>{members[0].areaName || area}</span><small>{shift === 'night' ? '夜班' : '早班'} · {date}</small></header><div className="dispatch-fields">{special ? <><b>人員</b><div>{members.map(record => <span className="person" key={record.id}>{record.employeeName}<small>{record.employeeId} · {record.scheduleCode}</small></span>)}</div></> : <><b>駕駛</b><div>{members.filter(record => record.driver).map(record => <span className="person" key={record.id}>{record.driver}<small>{record.employeeId} · {record.scheduleCode}</small></span>) || '—'}</div><b>隨車</b><div>{members.filter(record => record.assistant).map(record => <span className="person" key={record.id}>{record.assistant}</span>) || '—'}</div><b>駐點</b><div>{members.filter(record => record.station).map(record => <span className="person" key={record.id}>{record.employeeName}<small>{record.station}</small></span>) || '—'}</div><b>車型／車號</b><span>{members.map(record => `${record.vehicleType || '—'}／${record.vehicleNo || '—'}`).join('、')}</span><b>工作重點</b><span>{members[0].workFocus || '—'}</span><b>平衡區域</b><span>{members[0].balanceArea || '—'}</span><b>備註</b><span>{members.map(record => record.note).filter(Boolean).join('、') || '—'}</span></>}</div></article>})}</div>}</>
}

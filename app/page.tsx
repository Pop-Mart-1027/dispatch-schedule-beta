...
export default function Home() {
  ...
  useEffect(() => {
    if (currentUser && !currentUser.mustChangePassword && ['admin', 'duty'].includes(currentUser.role) && window.location.hash === '#vehicle-inbox') setPage('admin-console')
  }, [currentUser?.employeeId, currentUser?.mustChangePassword])
  ...
  if (page === 'admin-console' && (currentUser.role === 'admin' || currentUser.role === 'duty')) return <AdminConsole employeeId={account} employeeName={currentUser.name} role={currentUser.role} onExit={() => setPage('home')} onSignOut={() => void signOut(auth)} />
...
function AttendanceView({ employeeId, employeeName, scheduleData }: { employeeId: string; employeeName: string; scheduleData: ScheduleData | null }) {
  ...
  const punch = (type: PunchType) => { const block = getPunchBlockReason(records, type, taipeiToday()); if (checking || block) { setGeoMessage(block || '處理中，請稍候'); return } if (!context.hasSchedule) { setGeoMessage(context.scheduleCode ? '今日尚未派工' : '今日無排班'); return } if (!context.hasDispatch) { setGeoMessage('今日尚未派工，禁止正式打卡'); return } setChecking(true); setGeoMessage('正在取得 定位資訊…'); setGeoResult(null); if (!navigator.geolocation) { setGeoMessage('請開啟裝置定位權限後再試'); setChecking(false); return } navigator.geolocation.getCurrentPosition(async position => { const result = evaluateGeofence({ latitude: position.coords.latitude, longitude: position.coords.longitude }, locations.filter(location => location.areaCode === context.dispatchAreaCode)); setGeoResult(result); const accuracyWarning = position.coords.accuracy > GEOFENCE_WARNING_ACCURACY_METERS ? `定位精度較差（${Math.round(position.coords.accuracy)}公尺），結果僅供參考。` : ''; if (!result.canPunch) setGeoMessage(`${accuracyWarning} 此區域尚未設定打卡地點，或目前不在範圍內，禁止打卡`); else { const record = buildAttendanceRecord({ employeeId, employeeName, punchType: type, scheduleCode: context.scheduleCode, dispatchAreaCode: context.dispatchAreaCode, position: { latitude: position.coords.latitude, longitude: position.coords.longitude, accuracy: position.coords.accuracy }, evaluation: result }); try { await createAttendanceRecord(record); setRecords(current => [record, ...current]); setGeoMessage(`${accuracyWarning} ${type}打卡成功`) } catch { setGeoMessage('打卡紀錄寫入失敗，請確認網路後重試') } } setChecking(false) }, error => { setGeoMessage(error.code === error.PERMISSION_DENIED ? '定位權限被拒絕，請在瀏覽器設定中允許定位後重試' : `無法取得 GPS：${error.message}`); setChecking(false) }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }) }
  const contextMessage = !context.hasSchedule ? (context.scheduleCode ? '今日尚未派工' : '今日無排班') : !context.hasDispatch ? '今日尚未派工' : ''
  return <section className="form-card"><p className="eyebrow">打卡</p><h1>打卡</h1><p className="muted">{contextMessage || '打卡前會取得定位，並依今日派工區域判定電子圍籬。'}</p>{contextMessage && <div className="result-card result-warning">{contextMessage}</div>}<div className="quick-grid"><button className="quick-card" disabled={checking || Boolean(contextMessage) || Boolean(getPunchBlockReason(records, '上班', taipeiToday()))} onClick={() => punch('上班')}><span className="quick-icon start">↑</span><strong>上班打卡</strong><small>{getPunchBlockReason(records, '上班', taipeiToday()) || '取得定位後打卡'}</small></button><button className="quick-card" disabled={checking || Boolean(contextMessage) || Boolean(getPunchBlockReason(records, '下班', taipeiToday()))} onClick={() => punch('下班')}><span className="quick-icon end">↓</span><strong>下班打卡</strong><small>{getPunchBlockReason(records, '下班', taipeiToday()) || '取得定位後打卡'}</small></button></div>{geoMessage && <div className={`result-card ${geoResult?.canPunch ? 'result-success' : 'result-warning'}`}><strong>{geoMessage}</strong>{geoResult?.nearest ? <small>最近打卡點：{geoResult.nearest.checkpoint.name} · 距離 {Math.round(geoResult.nearest.distanceMeters)}公尺 · 允許半徑 {geoResult.nearest.checkpoint.radiusMeters}公尺</small> : <small>找不到此派工區域的啟用打卡點</small>}</div>}<div className="table-card"><h3>今日打卡紀錄</h3>{(['上班', '下班'] as PunchType[]).map(type => { const record = records.find(item => item.date === taipeiToday() && item.punchType === type); return <div className="list-row" key={type}><span><strong>{type}打卡</strong><small>{record?.locationName || '尚未打卡'} {record?.status === 'abnormal' ? '· 異常' : ''}</small></span><time>{record ? new Date(record.timestamp).toLocaleTimeString('zh-TW') : '—'}</time></div> })}</div></section>
}
...
function CheckpointAdmin() {
  ...
  const load = async () => { setLoading(true); try { setPoints(await listAttendanceLocations()); setError('') } catch (cause) { setError('打卡點讀取失敗，請確認權限與網路') } finally { setLoading(false) } }
  useEffect(() => { void load() }, [])
...
function GeofenceTest() {
  ...
  const test = () => { const point = points.find(item => item.id === selected); if (!point) return setStatus('請先選擇啟用中的打卡點'); if (!navigator.geolocation) return setStatus('此瀏覽器不支援定位'); setStatus('定位中…'); navigator.geolocation.getCurrentPosition(value => { const evaluation = evaluateGeofence({ latitude: value.coords.latitude, longitude: value.coords.longitude }, [point]); const distance = evaluation.nearest?.distanceMeters ?? 0; setResult({ position: `${value.coords.latitude.toFixed(6)}, ${value.coords.longitude.toFixed(6)}`, distance, accuracy: value.coords.accuracy, inside: distance <= point.radiusMeters }); setStatus('測試完成') }, error => setStatus(error.code === error.PERMISSION_DENIED ? '定位權限被拒絕，請允許定位後重試' : `定位失敗：${error.message}`), { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }) }
  const point = points.find(item => item.id === selected)
  return <section className="form-card"><p className="eyebrow">電子圍籬測試</p><h1>電子圍籬測試結果</h1><select value={selected} onChange={event => setSelected(event.target.value)}><option value="">選擇打卡點</option>{points.map(item => <option value={item.id} key={item.id}>{item.areaCode} · {item.locationName}</option>)}</select>{point && <p className="muted">打卡點座標：{point.latitude}, {point.longitude} · 允許半徑：{point.radiusMeters}公尺</p>}<button className="primary" onClick={test}><LocateFixed size={17} />取得目前位置並測試</button><div className={`result-card ${result?.inside ? 'result-success' : 'result-warning'}`}><strong>{result ? (result.inside ? '範圍內' : '範圍外') : status}</strong>{result && <small>目前座標：{result.position}<br />定位精度：{Math.round(result.accuracy)}公尺<br />實際距離：{Math.round(result.distance)}公尺<br />允許半徑：{point?.radiusMeters}公尺</small>}</div></section>
}

function IntegrationsView() { const items = [['打卡紀錄後端', '尚未串接資料庫與後端服務', '尚未串接'], ['請假申請與簽核', '目前僅有畫面示意', '尚未串接'], ['預排班儲存', '目前只在畫面中產生草稿', '尚未串接'], ['推播與異常通知', '尚未設定通知服務', '尚未串接'], ['正式電子圍籬驗證', '目前只有瀏覽器定位測試', '測試中']]; return <section className="form-card"><p className="eyebrow">功能狀態</p><h1>尚未串接的部分</h1><div className="table-card">{items.map(([name, detail, state]) => <div className="list-row" key={name}><span><strong>{name}</strong><small>{detail}</small></span><em>{state}</em></div>)}</div></section> }
...
function EmptyNotice() {
  ...
  return <section className="notice-page"><div className="notice-heading"><Megaphone size={25} /><div><p className="eyebrow">公告</p><h1>公告</h1></div></div><article className="notice-image-card"><img src={imageUrl || publicAssetUrl('temporary-notice.png')} alt="公告圖片" /></article></section>
}

function DispatchBlockPeople({ people, employeeId }: { people: DispatchBlockPerson[]; employeeId?: string }) {
  return <div>{people.length ? people.map(person => <span className={`person${employeeId && person.employeeId === employeeId ? ' dispatch-self-person' : ''}`} key={`${person.employeeId}-${person.employeeName}`}>{person.employeeName}<small>{person.employeeId || '待確認員編'}</small></span>) : '—'}</div>
}

type DutyStaff = { directors: string[]; deputyDirectors: string[]; taipeiMonitors: string[]; newTaipeiMonitors: string[] }

function deriveDutyStaff(records: ScheduleRecord[], profiles: EmployeeProfile[], shift: DispatchShift): DutyStaff {
  ...
  records.filter(record => parseDispatchShifts(record.scheduleCode).includes(shift) && !isLeave(record.scheduleCode)).forEach(record => {
...
function DutyStaffPanel({ staff, shift }: { staff: DutyStaff; shift: DispatchShift }) {
  const names = (items: string[]) => items.length ? items.join('、') : '未排定'
  return <section className="duty-staff" aria-label="值班資訊"><header><strong>值班資訊</strong></header><div>{shift === '早' && <><b>調度主任</b><span>{names(staff.directors)}</span><b>調度副主任</b><span>{names(staff.deputyDirectors)}</span></>}{monitorDisplayRows(staff.taipeiMonitors, staff.newTaipeiMonitors, shift === '早' ? 3 : 1).map(({ label, name }) => <Fragment key={`${label}-${name}`}><b>{label}</b><span>{name}</span></Fragment>)}</div></section>
}

function DispatchBackToTop() {
  const [visible, setVisible] = useState(false)
  useEffect(() => {
    const mobile = window.matchMedia('(max-width: 760px)')
    const content = document.querySelector<HTMLElement>('.app-shell[data-page="dispatch"] .dispatch-scroll-region')
  ...
  return visible ? <button type="button" className="dispatch-back-to-top" aria-label="回到頂部" onClick={() => {
    const target = document.documentElement.hasAttribute('data-app-viewport')
      ? document.querySelector<HTMLElement>('.app-shell[data-page="dispatch"] .dispatch-scroll-region') : window
...
function FirestoreDispatchView({ employeeId, isDuty }: { employeeId: string; isDuty: boolean }) {
  ...
  const [blocksLoadedDate, setBlocksLoadedDate] = useState('')
  ...
  const allDisplayBlocks = useMemo(() => {
    ...
    const assigned = buildShiftDispatchBlocks({
      date, blocks, schedules,
  ...
  const allVisible = useMemo(() => {
    ...
    const result = displayBlocks.map(block => dispatchAreaDisplay(block, validDispatchAreas)).filter(block => block.dispatchShift === shift && (isDuty || block.drivers.length + block.stations.length + block.assistants.length > 0)).sort(dispatchBlockFrontOrder)
  ...
  const ownBlocks = useMemo(() => isDuty ? [] : allDisplayBlocks
    ...
    .sort((a, b) => (dispatchShifts.indexOf(a.dispatchShift) - dispatchShifts.indexOf(b.dispatchShift)) || dispatchBlockFrontOrder(a, b)), [allDisplayBlocks, blocks, employeeId, isDuty, date])
  ...
  const displayCards = useMemo(() => pairDispatchCards(visible), [visible])
  useEffect(() => {
  ...
  useEffect(() => {
    if (isDuty || blocksLoadedDate !== date || peopleLoadedDate !== date || blocksLoading || dutyLoading || blocksError || dutyError) return
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const card = document.getElementById(scheduleSectionId('front-dispatch', date + '-' + shift, displayCards.find(card => card.sourceBlockIds.includes(scrollTarget.id))?.id || scrollTarget.id))
  ...
  useLayoutEffect(() => {
    ...
    const card = document.getElementById(scheduleSectionId('front-dispatch', date + '-' + shift, displayCards.find(card => card.sourceBlockIds.includes(own.id))?.id || own.id))
  ...
  const jumpAreas=useMemo(()=>{const areas=new Map();for(const block of displayCards){if(block.areaCode&&!areas.has(block.areaCode))areas.set(block.areaCode,{key:block.id,areaCode:block.areaCode,label:block.areaName || `${block.areaCode}區`});}return [...areas.values()]},[displayCards])
  ...
  return <><div className="dispatch-toolbar"><label>日期<input type="date" value={date} onChange={event => event.target.value && setDate(event.target.value)} /></label><div className="tabs"><>{dispatchShifts.map(item => <button key={item} className={shift === item ? 'tab active' : 'tab'} onClick={() => setShift(item)}>{item}班</button>)}</></div><AreaJumpDropdown areas={jumpAreas} group={`${date}-${shift}`} scope="front-dispatch" scrollTarget={dispatchRef} scrollMode="page" /></div><div className="dispatch-scroll-region">{!personalFirst && !isDuty && !blocksLoading && !dutyLoading && ownBlocks.length > 0 && <div className="dispatch-self-navigation" aria-label="我的派工定位"><span>你{date === taipeiToday() ? '今天' : '在此日'}有 {ownBlocks.length} 筆派工</span>{ownBlocks.length > 1 && <><button disabled={ownIndex === 0} onClick={() => selectOwn(ownIndex - 1)}>上一筆</button><span>{ownIndex + 1} / {ownBlocks.length}</span><button disabled={ownIndex === ownBlocks.length - 1} onClick={() => selectOwn(ownIndex + 1)}>下一筆</button></>}</div>}{personalFirst && !blocksLoading && !dutyLoading && !dispatchError && blocksLoadedDate === date && peopleLoadedDate === date && ownBlocks.length === 0 && <p className="dispatch-self-empty" role="status">今日尚無派工</p>}{dutyLoading ? <div className="duty-staff-status">值班資訊載入中…</div> : dutyError ? <div className="result-card result-warning">{dutyError}</div> : <DutyStaffPanel staff={dutyStaff} shift={shift} />}{dispatchError && <div className="result-card result-warning">{dispatchError}</div>}{dispatchLoading ? <p className="loading">{personalFirst ? '正在載入你的派工…' : '派工資料載入中…'}</p> : !dispatchError && !visible.length && (!personalFirst || fullListReady) ? <p className="loading">此日期尚無{shift}班派工區塊。</p> : <div className="dispatch-grid" ref={dispatchRef}>{displayCards.map(block => <article className={`dispatch-card${block.areaCode ? '' : ' command-card'}`} key={block.id} id={scheduleSectionId('front-dispatch',`${date}-${shift}`,block.id)} data-area-code={block.areaCode || undefined}><header><span>{block.areaName || block.areaCode || '特殊派工'}</span></header><div className="dispatch-fields"><b>車號</b><span>{block.vehicleNo || '—'}</span><b>駕駛</b><DispatchBlockPeople people={block.drivers} employeeId={isDuty ? undefined : employeeId} /><b>駐點</b><DispatchBlockPeople people={block.stations} employeeId={isDuty ? undefined : employeeId} />{block.assistants.length > 0 && <><b>隨車</b><DispatchBlockPeople people={block.assistants} employeeId={isDuty ? undefined : employeeId} /></>}<b>工作重點</b><WorkFocus text={block.workFocus} /></div></article>)}</div>}{personalFirst && !fullListReady && !dispatchError && !blocksLoading && <p className="dispatch-loading-rest" role="status">{visible.length ? '正在載入其餘派工…' : dutyLoading ? '正在載入你的派工…' : '正在載入完整派工單…'}</p>}<DispatchBackToTop /></div></>
}
...
function ProfileView({ employee }: { employee: Employee }) { return <article className="form-card profile-card"><p className="eyebrow">個人資料</p><h1>個人資料</h1><dl><div><dt>姓名</dt><dd>{employee.name}</dd></div><div><dt>員工編號</dt><dd>{employee.employeeId}</dd></div><div><dt>職稱</dt><dd>{employee.title}</dd></div><div><dt>到職日</dt><dd>{employee.hireDate}</dd></div><div><dt>帳號狀態</dt><dd>{employee.active ? '啟用' : '停用'}</dd></div></dl></article> }
...
function ChangePassword({ employee, onDone }: { employee: Employee; onDone: () => Promise<void> }) {
  ...
  return <main className="login-page"><section className="login-card password-card"><KeyRound className="password-icon" size={36} /><p className="eyebrow">帳號安全</p><h1>請先設定新密碼</h1><p className="muted">首次登入或管理員重設後，必須修改密碼才能進入工作台。</p><label>新密碼<input type="password" autoComplete="new-password" value={newPassword} onChange={event => setNewPassword(event.target.value)} /></label><label>再次輸入新密碼<input type="password" autoComplete="new-password" value={confirmation} onChange={event => setConfirmation(event.target.value)} onKeyDown={event => event.key === 'Enter' && void submit()} /></label><button className="primary full" disabled={saving} onClick={() => void submit()}>{saving ? '更新中…' : '更新密碼並重新登入'}</button>{error && <p className="error">{error}</p>}<button className="text-button" onClick={() => void signOut(auth)}>登出</button></section></main>
}

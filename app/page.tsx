...
export default function Home() {
  ...
  const titles: Record<string, string> = { home: '工作總覽', notice: '公告', attendance: '打卡', checkpoints: '打卡點管理', geofence: '電子圍籬測試', schedule: '我的班表', dispatch: '派工單', pre: '預排班', leave: '假勤／特休', profile: '個人資料', employees: '員工管理', integrations: '尚未串接' }
  const go = (target: string) => { setPage(target); setMenuOpen(false) }
  return <div className="app-shell" data-page={page}>{menuOpen && <button className="mobile-backdrop" aria-label="關閉選單" onClick={() => setMenuOpen(false)} />}<aside className={menuOpen ? 'sidebar open' : 'sidebar'}><div className="logo"><span>調</span><div><strong>調度工作台</strong><small>DISPATCH BETA</small></div><button className="drawer-close" aria-label="關閉選單" onClick={() => setMenuOpen(false)}><X size={20} /></button></div><nav><Nav label="工作總覽" active={page === 'home'} icon={<Coffee size={18} />} onClick={() => go('home')} /><Nav label="打卡" active={page === 'attendance'} icon={<CheckCircle2 size={18} />} onClick={() => go('attendance')} /><Nav label="公告" active={page === 'notice'} icon={<Megaphone size={18} />} onClick={() => go('notice')} /><Nav label="我的班表" active={page === 'schedule'} icon={<CalendarDays size={18} />} onClick={() => go('schedule')} /><Nav label="派工單" active={page === 'dispatch'} icon={<ClipboardList size={18} />} onClick={() => go('dispatch')} /><Nav label="預排班" active={page === 'pre'} icon={<Clock3 size={18} />} onClick={() => go('pre')} /><Nav label="假勤／特休" active={page === 'leave'} icon={<ClipboardPlus size={18} />} onClick={() => go('leave')} /><Nav label="個人資料" active={page === 'profile'} icon={<UserRound size={18} />} onClick={() => go('profile')} />{currentUser.role === 'admin' && <><Nav label="打卡點管理" active={page === 'checkpoints'} icon={<MapPin size={18} />} onClick={() => go('checkpoints')} /><Nav label="圍籬測試" active={page === 'geofence'} icon={<LocateFixed size={18} />} onClick={() => go('geofence')} /><Nav label="員工管理" active={page === 'employees'} icon={<ShieldCheck size={18} />} onClick={() => go('employees')} /></>}<Nav label="尚未串接" active={page === 'integrations'} icon={<Clock3 size={18} />} onClick={() => go('integrations')} /></nav><button className="logout" onClick={() => void signOut(auth)}><LogOut size={17} />登出</button></aside><main className="workspace"><header className="topbar"><button className="menu-button" aria-label="開啟選單" onClick={() => setMenuOpen(true)}><Menu size={22} /></button><div><p className="eyebrow">2026 年 9 月</p><h2>{titles[page]}</h2></div><div className="profile-chip"><b>{currentUser.name[0]}</b><span>{currentUser.name}<small>{account} · {currentUser.role === 'admin' ? '管理員' : '調度專員'}</small></span></div></header><section className="content">{page === 'home' && <HomeView onAction={notify} name={currentUser.name} onGo={go} />}{page === 'notice' && <EmptyNotice />}{page === 'attendance' && <AttendanceView employeeId={account} employeeName={currentUser.name} scheduleData={scheduleData} />}{page === 'checkpoints' && currentUser.role === 'admin' && <CheckpointAdmin />}{page === 'geofence' && currentUser.role === 'admin' && <GeofenceTest />}{page === 'integrations' && <IntegrationsView />}{page === 'schedule' && <ScheduleView tab={scheduleTab} setTab={setScheduleTab} data={scheduleData} employeeId={account} />}{page === 'dispatch' && <DispatchView data={scheduleData} />}{page === 'pre' && <PreSchedule onSave={() => notify('預排班已儲存為草稿')} />}{page === 'leave' && <LeaveView onAction={notify} />}{page === 'profile' && <ProfileView employee={currentUser} />}{page === 'employees' && currentUser.role === 'admin' && <AdminEmployees notify={notify} />}</section></main>{notice && <div className="toast">✓ {notice}</div>}</div>
}
...
function HomeView({ onAction, name, onGo }: { onAction: (text: string) => void; name: string; onGo: (page: string) => void }) { return <><section className="welcome"><div><p className="eyebrow">WELCOME BACK</p><h1>你好，{name}。</h1><p className="muted">今天也一起順利出勤。</p></div><BikeArtwork /></section><section className="quick-section"><div><p className="eyebrow">QUICK ACTIONS</p><h2>快速選單</h2></div><div className="quick-grid"><button className="quick-card" onClick={() => onGo('attendance')}><span className="quick-icon start">↑</span><strong>上班打卡</strong><small>記錄今日上班時間</small></button><button className="quick-card" onClick={() => onGo('attendance')}><span className="quick-icon end">↓</span><strong>下班打卡</strong><small>記錄今日下班時間</small></button><button className="quick-card" onClick={() => onAction('請假申請尚未串接後端')}><span className="quick-icon leave">＋</span><strong>我要請假</strong><small>病假、事假與特休申請</small></button></div></section></> }
